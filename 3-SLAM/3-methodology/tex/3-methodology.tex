...
To begin this investigation, GLIM \cite{koideGLIM3DRangeInertial2024} was selected as the base framework. GLIM was chosen because it provides infrastructure to add extension modules incorporating new sensors and capabilities. GLIM provides a global callback slot mechanism that allows access to the internal states of the mapping process and insertion of additional constraints into the factor graph. GLIM also eliminated sensor-specific processes, meaning it can be applied to a wide variety of range sensors such as spinning-type LiDAR, non-repetitive scan LiDAR, solid-state LiDAR and even RGB-D cameras.

GLIM state estimation is created in the local odometry frame, whose origin is determined by wherever the robot begins mapping. Without an absolute reference, each run produces a distinct, arbitrarily-oriented map frame, making trajectory estimates across runs mutually incomparable. For use in the wider research project, state estimation in a consistent local ENU reference frame surveyed at each farm was required, so that all localisation data — regardless of when or where a run begins — can be expressed in the same coordinate system and compared directly. This necessitated a procedure to align the GLIM map frame with the existing local ENU reference frame.

Firstly, the on-board GPS unit is time-synchronised with the LiDAR and with the system clock. This allows corresponding GNSS points to be associated with the submaps or keyframes of LiDAR frames that GLIM registers, as illustrated in \cref{fig:time-matching}. A set of corresponding GNSS coordinates and keyframe coordinates is aggregated, and the Kabsch algorithm \cite{kabschSolutionBestRotation1976} is applied to find the relative transform between the two point sets by minimising the root-mean-square deviation between them.
...
To remove spurious GNSS measurements prior to initialisation, a geometric consistency check is applied that exploits the rigidity of the GNSS--IMU transformation. The method assumes that, for inlier measurements, pairwise distances between GNSS observations should be preserved when mapped into the local map frame via the estimated poses.

Let $\{\mathbf{g}_i\}_{i=1}^{N}$ denote GNSS positions expressed in the ENU frame, and $\{\hat{\mathbf{g}}_i\}_{i=1}^{N}$ denote the corresponding GNSS positions expressed in the map frame, obtained by transforming IMU poses with a fixed extrinsic calibration. For each point $i$, its consistency with all other points $j \neq i$ is evaluated by comparing pairwise Euclidean distances in both frames as shown in Equation \eqref{eq:consistency_check}:
\begin{equation}
...
To formalise the inlier classification, a pairwise consistency indicator $c_{ij}$ is defined in Equation \eqref{eq:indicator}:
\begin{equation}
\label{eq:indicator}
c_{ij} = 
\begin{cases} 
...
Subsequently to outlier rejection, the geometric quality of the remaining point set is assessed under the assumption of planar motion by analysing its distribution in the horizontal plane. This analysis is motivated by the sensitivity of the Kabsch algorithm to degenerate point configurations: near-collinear or poorly distributed trajectories produce ill-conditioned cross-covariance matrices, leading to unstable or biased transformations. By detecting such configurations before alignment, the SVD is ensured to be well-conditioned and the estimated transformation numerically reliable. In particular, minimum requirements on translational baseline ($\geq 5.0\,\mathrm{m}$), planar spread, rotational excitation, and numerical conditioning are enforced.

Given a set of 3D points $\{\mathbf{p}_i\}_{i=1}^{N}$, only the horizontal components are considered by projecting each point onto the $xy$-plane, yielding $\mathbf{q}_i = [x_i, y_i]^\top$. If fewer than three points are available, the geometry is deemed insufficient and the assessment terminates early. The centred projected points are analysed via SVD to obtain the two singular values $\sigma_1 \geq \sigma_2$ that characterise the principal spatial extents of the trajectory in the plane. The condition number
%
\begin{equation}
...
\end{equation}
%
serves as the primary degeneracy metric: large values indicate near-collinear motion, and the geometry is considered non-degenerate if $\kappa < 20.0$. Spatial coverage is assessed via the spread ratio $r = \sigma_2 / \sigma_1$, which must satisfy $r \geq 0.15$, and both singular values must exceed a baseline displacement of $5.0\,\mathrm{m}$. Rotational excitation is evaluated independently by requiring the angular span of the point set in the horizontal plane to exceed $45.0^\circ$.
...
To explore the initial parameter manifold, Latin Hypercube Sampling (LHS) is used, following the stratified sampling strategy described in \cref{subsec:slam-hyperparameter}.
...
At the conclusion of each phase $i$, the top-performing configurations are used to calculate new search boundaries for phase $i+1$. The 10th and 90th percentiles of the elite set serve as robust estimators for the update rule in Equation \eqref{eq:quantile_update}:
\begin{equation}
\label{eq:quantile_update}
\Omega_{j, i+1} = [P_{10}(\theta_{j, \text{top}}), P_{90}(\theta_{j, \text{top}})].
\end{equation}
This quantile-based reduction is superior to using absolute Min/Max values because it is resilient to ``lucky'' outliers, specifically configurations that may perform well due to noise interactions rather than parameter stability.
