...
Firstly, we time synchronise the on-board GPS unit with the LiDAR and with the system clock. This allows us to associate corresponding GNSS points with the submaps or keyframes of LiDAR frames that GLIM registers. We aggregate a set of corresponding GNSS coordinates with keyframe coordinates, and then implement the Kabsch algorithm to find the relative transform between the two point sets by minimising the root-mean-square deviation between the two point sets.
...
GNSS multipath errors can often create erroneous clusters of points that are mutually consistent, and therefore are not filtered out by conventional statistical outlier methods. However, the majority-based criterion in Equation \eqref{eq:majority_criterion} ensures that points are only kept if they belong to the global rigid consensus, effectively filtering out clusters of mutually consistent but globally erroneous points. After classification, only inlier GNSS measurements and their corresponding map-frame estimates are preserved for subsequent processing.
\subsection{Planar Geometry Quality Assessment in 2D}

Subsequently to outlier rejection, we assess the geometric quality of the remaining set of points under the assumption of planar motion by analyzing their distribution in the horizontal plane. This analysis is motivated by the use of the Kabsch algorithm for rigid alignment, which relies on singular value decomposition (SVD) of the cross-covariance matrix and is highly sensitive to degenerate or weakly excited point configurations. Trajectories that are near-collinear, poorly distributed, or exhibit insufficient translational or rotational excitation can lead to ill-conditioned covariance matrices, resulting in unstable or biased pose estimates. By explicitly detecting such configurations prior to alignment, we ensure that the point sets provide sufficient geometric constraints to yield a well-conditioned SVD and improve the numerical stability and accuracy of the estimated transformation. In particular, we enforce minimum requirements on translational baseline ($\geq 5.0\,\mathrm{m}$), planar spread, rotational excitation, and numerical conditioning.
...
\end{equation}
which is centered by subtracting the mean $\bar{\mathbf{q}}$ from each column. Singular value decomposition is then applied to the centered matrix as shown in Equation \eqref{eq:svd}:
\begin{equation}
\label{eq:svd}
\mathbf{M} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top,
\end{equation}
yielding two singular values $\sigma_1 \geq \sigma_2$ that characterize the principal spatial extents of the trajectory in the plane.
...
Given two corresponding point sets $\{\mathbf{a}_i\}_{i=1}^N, \{\mathbf{b}_i\}_{i=1}^N \subset \mathbb{R}^3$, the goal is to estimate a rigid-body transformation $(\mathbf{R}, \mathbf{t}) \in \mathrm{SO}(3) \times \mathbb{R}^3$ that minimizes the sum of squared alignment errors as defined in the following objective function:
\begin{equation}
...
The point sets are then centered by subtracting their respective centroids:
\begin{equation}
\label{eq:centering}
...
The resulting rigid transformation $(\mathbf{R}, \mathbf{t})$ aligns the source point set to the target point set in a least-squares sense. This static transformation is published to the ROS2 transform tree to link the local ENU frame to the GLIM mapping frame.

\subsection{GNSS Prior Factor Construction}

This procedure augments an existing SLAM factor graph with GNSS-based prior constraints that relate submap poses to global position measurements. Each GNSS measurement is incorporated as a probabilistic constraint on the estimated submap pose through a known sensor extrinsic calibration.
...
\subsection{Methodology: Robust SLAM Parameterization via Iterative Region Refinement}

The optimisation of Simultaneous Localisation and Mapping (SLAM) parameters is a high-dimensional, non-convex problem where the objective is to identify a parameter vector $\Theta$ that minimises localisation error across heterogeneous environments[cite: 1]. This paragraph proposes a model-free, heuristic region-refinement algorithm that transitions from global stochastic exploration to localised deterministic refinement[cite: 2]. The strategy assumes that optimal configurations cluster within specific sub-regions of the hyper-dimensional search space $\Omega$, which are iteratively identified and contracted using statistical distribution analysis[cite: 3].

\paragraph{Problem Formulation}
We define the SLAM system as a complex function $f(X,\theta)$, where $X$ represents raw sensor data (e.g., LiDAR scans and inertial measurements) and $\Theta$ is the $d$-dimensional parameter vector[cite: 4]. In a typical LiDAR-inertial setup, $\Theta$ includes parameters such as LiDAR resolution and registration strategy. The objective is to find $\Theta^*$ such that:
\begin{equation}
    \Theta^* = \arg\min_{\Theta \in \Omega} \mathcal{J}(\Theta)
\end{equation}
where $\mathcal{J}$ is a robust objective function representing the consensus performance across $K$ distinct motion profiles[cite: 6].

\paragraph{Sampling via Latin Hypercube Strategy}
To explore the initial parameter manifold, we utilise Latin Hypercube Sampling (LHS) as an alternative to traditional grid search or independent Monte Carlo (MC) sampling[cite: 7]. LHS is technically categorised as a stratified sampling design[cite: 8]. For a sample size $N$ in $d$ dimensions, the range of each parameter $\theta_j$ is partitioned into $N$ equiprobable intervals for $j=1, \dots, d$[cite: 9]. The mathematical constraint --- ensuring exactly one sample per row and column of the grid --- minimises the ``clustering'' phenomenon common in pure random sampling[cite: 10]. This stratification ensures that the marginal distribution of each parameter is sampled uniformly, providing a convergence rate of $o(N^{-1/2})$, which is quadratically faster than the rate of Monte Carlo methods[cite: 11].

\subparagraph{Implementation and Normalization}
Parameter vectors are first normalised to the unit hypercube $[0, 1]^d$[cite: 12]. To ensure reproducibility, the pseudo-random permutations used to construct the hypercube are seeded deterministically[cite: 13]. The samples are subsequently scaled to physical ranges using:
\begin{equation}
    \theta_{j,i} = \theta_{j, \min} + u_{j,i} (\theta_{j, \max} - \theta_{j, \min})
\end{equation}
where $\theta_{j, \min}$ and $\theta_{j, \max}$ represent the physical lower and upper bounds for parameter $j$[cite: 14].

\paragraph{The Multi-Dataset Consensus Framework}
A primary challenge in SLAM optimisation is the risk of ``over-specialisation'', where parameters become overfitted to the unique noise characteristics of a single trajectory[cite: 15]. We implement a Consensus Survival Framework to prioritise reliability across datasets[cite: 16].

In some situations, standard ``Mean-Performance Optimisation'' can hide brittle configurations that perform well on average but fail catastrophically on a single sequence[cite: 17]. However, in this setting, catastrophic failure typically results in excessively high amplitude errors, which effectively inflate the mean normalised error. That being said, some particular parameter configurations can lead to excessive GPU draw, resulting in a runtime CUDA out-of-memory error. If the parameter set does not complete processing the trajectory on any dataset $k$, it is assigned an infinite cost and discarded from the survivor pool[cite: 19]. This ensures the survival of only those configurations that meet the computational requirements of the system[cite: 20].

\subparagraph{Formal Objective Function}
...
\paragraph{Iterative Region Refinement (IRR)}
The optimisation proceeds through an ``Iterative Contraction'' or ``Funnel'' strategy \cite{cite:22}. This mirrors the reduction of a search radius in traditional Trust Region Methods, concentrating the sampling density into the most promising basins of attraction \cite{cite:23}.

\subparagraph{Statistical Search Space Contraction}
At the conclusion of each phase $i$, the top-performing configurations are used to calculate new search boundaries for phase $i+1$ \cite{cite:24}. We employ the 10th and 90th percentiles of the elite set as robust estimators for the update rule in Equation \eqref{eq:quantile_update}:
\begin{equation}
\label{eq:quantile_update}
\Omega_{j, i+1} = [P_{10}(\theta_{j, \text{top}}), P_{90}(\theta_{j, \text{top}})].
\end{equation}
This quantile-based reduction is superior to using absolute Min/Max values because it is resilient to ``lucky'' outliers --- configurations that may perform well due to noise interactions rather than parameter stability \cite{cite:25}.

\subparagraph{Refinement Phase Schedule}
The optimisation is structured into four phases, increasing the selection pressure (lower survival rate) as the search space contracts \cite{cite:26}, as detailed in Table \ref{tab:phases}:
...
This incremental strategy is distinct from Successive Halving (SHA), as it preserves a continuous distribution of potential solutions rather than discarding them in discrete early-stopping steps, which is critical for SLAM where early performance does not always correlate with final drift \cite{cite:28}.

\paragraph{Optimisation Metrics}

For the optimisation of the base GLIM algorithm, the cost function for candidate ranking is the Trajectory Error Percentage ($TEP$), calculated as the percentage of the ratio of Absolute Trajectory Error ($ATE$) to the total trajectory length ($L$) \cite{cite:29}:
\begin{equation}
\label{eq:tep}
...
\end{equation}
The weights $w_{\text{drift}}$ and $w_{\text{smoothness}}$ have been empirically tuned such that each term contributes approximately equally to the total cost. Due to the heuristic nature of these weighting factors, the $GASP$ metric functions as a relative performance index rather than an absolute physical measurement. While individual components possess physical units, their weighted summation yields a unitless scalar used to rank configurations; consequently, a $GASP$ value is only meaningful when benchmarked against a baseline or alternative configurations within the same experimental framework.
