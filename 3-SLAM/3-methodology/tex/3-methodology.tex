...
The optimisation proceeds through an ``Iterative Contraction'' or ``Funnel'' strategy. This mirrors the reduction of a search radius in traditional Trust Region Methods, concentrating the sampling density into the most promising basins of attraction.
...
This incremental strategy is distinct from Successive Halving (SHA), as it preserves a continuous distribution of potential solutions rather than discarding them in discrete early-stopping steps, which is critical for SLAM where early performance does not always correlate with final drift.

\subsubsection{Optimisation Metrics}

For the optimisation of the base GLIM algorithm, the cost function for candidate ranking is the Trajectory Error Percentage ($TEP$), calculated as the percentage of the ratio of Absolute Trajectory Error ($ATE$) to the total trajectory length ($L$):
\begin{equation}
\label{eq:tep}
