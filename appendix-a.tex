...
% ===== SLAM: Map Visualisations =====
\section{SLAM Map Visualisations}
\label{app:slam-map-figures}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{3-SLAM/3-intro/figures/slam_map_mid_zoom.png}
    \caption{Perspective view of the LiDAR point cloud from inside a vineyard row, captured after the robot has completed the forward pass and begun the return traversal on the adjacent row. The corridor geometry and row length are apparent; the symmetric canopy and ground returns on either side provide no discriminative features for longitudinal scan-matching.}
    \label{fig:slam-map-mid-zoom}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Trajectories in XY plane from baseline GLIM and GLIM-GNSS. Left column: baseline GLIM; right column: GLIM-GNSS; both compared against RTK~GNSS ground truth.}
...
\begin{figure}[H]
    ...
    \caption{Trajectories in Z plane from baseline GLIM and GLIM-GNSS. Left column: baseline GLIM; right column: GLIM-GNSS; both compared against RTK~GNSS ground truth.}
    \label{fig:large_grid_z}
\end{figure}

% ===== Precise Alignment: LiDAR Heatmap =====
\section{Precise Alignment}
