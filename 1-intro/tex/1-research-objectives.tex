...
\begin{enumerate}
    ...
    \item Develop a LiDAR-inertial SLAM system that achieves low-decimetre absolute trajectory error in GNSS-degraded vineyard environments, addressing the structural ambiguity of row-crop corridors.

    \item Design a precise-alignment pipeline that refines the SLAM pose to low-centimetre accuracy by registering live sensor data against a 3D Gaussian Splatting reference model using point cloud registration.
...
\begin{enumerate}
    \item A LiDAR-inertial SLAM system augmented with sparse GNSS factor injection (GLIM-GNSS) that achieves a mean absolute trajectory error of 103\,mm across five vineyard datasets while utilising less than 1\% of available GNSS measurements, demonstrating that low-decimetre localisation is attainable in repetitive row-crop environments without continuous RTK coverage.
